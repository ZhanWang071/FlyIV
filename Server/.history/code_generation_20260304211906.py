...
{"function": "UPDATE(string chart id, string element_id, string y_value)",
...
{"function" :"APPEND SERIES(string chart_id, List<string>x_values, List<string> y_values, int serieIndex, string serieType)",
"description":"Append a series of elements"}

{"function" :"HIGHLIGHT(string chart id,string element id, string highlight type)",
"description": "Highlight a single element"}
# 针对dxR, 在目标 mark GameObject 上动态创建 TextMesh label 子对象实现高亮; 不对颜色和shadow进行修改；`highlight_type = "none"` 时销毁所有 label。直接操作 Unity GameObject，不需要修改 visSpecs，不需要 UpdateVis.
