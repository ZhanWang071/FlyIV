...
{
    public static byte[] FromAudioClip(AudioClip clip, int lengthSamples, int startSamplePos, int targetSampleRate = 8000)
    {
        using (var stream = new MemoryStream())
        {
            float[] samples = new float[lengthSamples * clip.channels];
            clip.GetData(samples, startSamplePos);

            // 写入 WAV 头部
            WriteHeader(stream, clip, lengthSamples);
            // 写入 PCM 数据
    ...
    private static void WriteHeader(Stream stream, AudioClip clip, int samples)
    {
        ...
        int byteCount = samples * channels * 2;

        stream.Seek(0, SeekOrigin.Begin);

        stream.Write(System.Text.Encoding.UTF8.GetBytes("RIFF"), 0, 4);
        stream.Write(BitConverter.GetBytes(byteCount + 36), 0, 4);
    ...
    // public static byte[] FromAudioClip(AudioClip clip, int lengthSamples, int targetSampleRate = 8000)
    // {
    //     // 原始数据
    //     float[] samples = new float[lengthSamples * clip.channels];
    ...
    //     // 计算降采样后的采样点数
    //     int targetLength = (int)((float)lengthSamples * targetSampleRate / originalRate);
    ...
    //             int srcIndex2 = Mathf.Min(srcIndex1 + 1, lengthSamples - 1);
